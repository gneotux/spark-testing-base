...
object RDDGenerator {
  ...
  /**
    * Generate an RDD of the desired type with its size accessible in the lambda generator.
    * Attempt to try different number of partitions so as to catch problems with empty partitions, etc.
    *
    * minPartitions defaults to 1, but when generating data too large for a
    * single machine you must choose a larger value.
    *
  ...
  def genSizedRDD[T: ClassTag](
    sc: SparkContext, minPartitions: Int = 1)(generator: Int => Gen[T]): Gen[RDD[T]] = {
  ...
  def arbitrarySizedRDD[T: ClassTag](
                                      sc: SparkContext,
                                      minPartitions: Int = 1)(generator: Int => Gen[T]):
  Arbitrary[RDD[T]] = {
    Arbitrary {
      Gen.sized { sz =>
  ...
  sc: SparkContext,
  minPartitions: Int,
  sz: Int)(randomDataGenerator: => RandomDataGenerator[T])
    : Gen[RDD[T]] = {
    sz match {
...
private[testing] class WrappedSizedGenerator[T](size: Int, getGenerator: Int => Gen[T])
  extends WrappedGen[T] {
